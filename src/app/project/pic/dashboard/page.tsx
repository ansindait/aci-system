...
const Dashboard = () => {
  ...
  // Filtered site data based on logged-in user's PIC and other filters
  const filteredSiteData = useMemo(() => {
    let filtered = [...siteData];
    
    // Automatically filter by logged-in user's PIC name
    if (loginName) {
      filtered = filtered.filter(site => 
        (site.picPermit && site.picPermit.toLowerCase().includes(loginName.toLowerCase())) ||
        (site.picSnd && site.picSnd.toLowerCase().includes(loginName.toLowerCase())) ||
        (site.picCw && site.picCw.toLowerCase().includes(loginName.toLowerCase())) ||
        (site.picEl && site.picEl.toLowerCase().includes(loginName.toLowerCase())) ||
        (site.picDocument && site.picDocument.toLowerCase().includes(loginName.toLowerCase())) ||
        (site.pic && site.pic.toLowerCase().includes(loginName.toLowerCase()))
      );
    }
    ...
    return filtered;
  }, [siteData, loginName, dateRange, allTasks]);
  ...
  const filteredDetailsData = useMemo(() => {
    ...
    const result = filteredSiteData.filter(item => {
      ...
      // PIC filters are already applied in filteredSiteData
      ...
      const finalMatch = picMatch && dateMatch && cityMatch;
      if (!finalMatch) {
        console.log(`Site ${item.siteName} filtered out: picMatch=${picMatch}, dateMatch=${dateMatch}, cityMatch=${cityMatch}`);
      }
      ...
      const result = picMatch && dateMatch && cityMatch && pieMatch;
      if (selectedPieFilter) {
  ...
  }, [loginName, dateRange, siteData, selectedCity, selectedPieFilter, allTasks, searchTerm]);
                          ...
                          >
                            ...
                            <td className="p-2 text-center border border-gray-300 text-gray-800 text-sm whitespace-nowrap">
                              <div className="flex justify-center items-center space-x-3">
                                <button
                                  onClick={() => router.push(`/project/pic/site/preview?siteName=${encodeURIComponent(item.siteName)}`)}
