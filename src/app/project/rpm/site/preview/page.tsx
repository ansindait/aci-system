...
const SitePreviewContent = () => {
  ...
  // Fetch and filter sites by logged-in user (RPM)
  useEffect(() => {
    const fetchFilteredSites = async () => {
      ...
      // 1. Fetch all tasks where rpm matches user.name (case-insensitive)
      const tasksSnapshot = await getDocs(collection(db, "tasks"));
      const userNameLower = user.name.toLowerCase();
      const userTasks = tasksSnapshot.docs.filter(doc => {
        const data = doc.data();
        return (data.rpm || "").toLowerCase() === userNameLower;
      });
  ...
  useEffect(() => {
    const calculateTotalHP = async () => {
      try {
        // 1. Fetch all tasks (RPM can see tasks under their supervision)
        const tasksSnapshot = await getDocs(collection(db, "tasks"));
  ...
  const uploadSectionFile = async (
    ...
    try {
      ...
      if (!updatedSnap.empty) {
        ...
        // If original query is empty, try fallback query for refetch
        let fallbackQ;
        if (selectedSiteId) {
          fallbackQ = query(collection(db, "tasks"), where("siteId", "==", selectedSiteId));
        } else if (selectedSiteName) {
        ...
        if (fallbackQ) {
          const fallbackSnap = await getDocs(fallbackQ);
          if (!fallbackSnap.empty) {
            const updatedData = fallbackSnap.docs[0].data();
            setUploadedSections(Array.isArray(updatedData.sections) ? updatedData.sections : []);
          }
        }
      }
