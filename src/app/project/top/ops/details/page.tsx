...
const OpsDetailsPage = () => {
  ...
  const filteredRequestOps = allRequestOps
    ...
    .filter((item) => {
      const { siteId, region, city, siteName, date, pic, rpm } = filters;
      return (
        ...
        (!rpm || (item.parent.rpm || '').toLowerCase().includes(rpm.toLowerCase()))
      );
    });
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(workbook, 'ops_details_top.xlsx');
  };
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 flex-grow">
                ...
                {/* Dropdown Searchable: RPM */}
                <SearchableDropdown
                  label="RPM"
                  options={getUnique(allRequestOps, 'rpm')}
                  value={filters.rpm}
                  onChange={(val: string) => setFilters(f => ({ ...f, rpm: val }))}
                />
