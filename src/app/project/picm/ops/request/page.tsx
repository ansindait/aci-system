...
const RequestOpsPage = () => {
  ...
  // Fetch city options dari seluruh tasks
  useEffect(() => {
  ...
  // Fetch siteName options setiap kali city berubah
  useEffect(() => {
  ...
  // Fetch PIC options setiap kali city berubah
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchAllRequestOps = async () => {
      setLoading(true);
      try {
        // Fetch from request_ops_rpm collection filtered by division "pic manager"
        const requestOpsRpmQuery = query(
          collection(db, 'request_ops_rpm'),
          where('division', '==', 'pic manager')
        );
        const requestOpsRpmSnapshot = await getDocs(requestOpsRpmQuery);
        ...
        console.error('Error fetching request_ops_rpm:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // For PICM, we don't need to validate against tasks collection
      // They can submit OPS requests regardless of existing tasks
      ...
      // Tambahkan data ke collection request_ops_rpm
      let notaUrl = '';
      let nota = false;
      if (fileNota) {
      ...
      // Save to request_ops_rpm collection
      await setDoc(doc(collection(db, 'request_ops_rpm'), activityId), {
        date: form.date,
        activityId,
        siteName: form.siteName,
        city: form.city,
        division: 'pic manager', // Set division to "pic manager" for PICM
