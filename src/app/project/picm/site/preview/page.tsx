...
const SitePreviewContent = () => {
  ...
  // For PIC Manager, fetch ALL sites (no filtering by user)
  useEffect(() => {
    const fetchFilteredSites = async () => {
      // Fetch all sites without filtering by user
      const sitesSnapshot = await getDocs(collection(db, "site"));
      const allSites = sitesSnapshot.docs
  ...
  useEffect(() => {
    const calculateTotalHP = async () => {
      try {
        // 1. Fetch all tasks (PIC can see their own tasks)
  ...
  const uploadSectionFile = async (
    ...
    try {
      ...
      // Find the task document by siteId or siteName AND division (division harus huruf kecil)
      let q;
      let divisionToSearch = selectedSectionDivision.toLowerCase();
  ...
  const handleRemoveUpload = async (sectionKey: string, idx: number) => {
    ...
    try {
      ...
      // Hapus hanya pada section dan index yang sesuai
      let foundIdx = -1;
      let count = 0;
    ...
    try {
      ...
      for (let i = 0; i < allImageFiles.length; i++) {
        ...
        try {
          ...
          // Create folder structure: Division/Section/timestamp_filename
          const sectionName = file.section;
          const division = file.division || 'unknown';
          const timestamp = file.uploadedAt ? 
            new Date(file.uploadedAt.seconds * 1000).toISOString().replace(/[:.]/g, '-') : 
            Date.now().toString();
          const fileName = `${timestamp}_${file.fileName}`;
          const zipPath = `${division}/${sectionName}/${fileName}`;
      ...
      a.href = url;
      a.download = `Photos_${selectedSiteId || selectedSiteName}_${new Date().toISOString().split('T')[0]}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      
      console.log(`ZIP exported with ${allImageFiles.length} images`);
    } catch (error) {
                ...
                {isOpen[sectionKey] && (
                  <div className="p-6 bg-white border border-gray-200 rounded-b-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      {/* Show upload card for Material division regardless of status, or for other divisions if status is Not Yet */}
                      {(item.division === 'Material' || (item.status !== 'Done' && item.division !== 'Material')) && !pendingUpload[sectionKey] && (
                        <div
