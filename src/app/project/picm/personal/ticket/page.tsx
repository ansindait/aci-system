...
const TicketRequestPage = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(collection(db, 'ticket_request'), (snapshot) => {
      // Filter tickets to only show current user's tickets
      const userTickets = snapshot.docs
        .map(doc => ({ id: doc.id, ...doc.data() } as any))
        .filter((ticket: any) => ticket.nik === form.nik || ticket.userEmail === user.email);
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          setForm(prevForm => ({
            ...
            position: prevForm.position || getPositionDisplay(positionValue),
          }));
        }
      } catch (error) {
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow-md">
            ...
            >
              ...
              <span className="font-semibold text-lg">Ticket History</span>
            </button>
            {isOpen && (
