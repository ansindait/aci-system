...
async function getHPData(loginName: string): Promise<{ totalHP: number; hpData: Array<{ siteName: string; siteId: string; hp: number; city: string; status: string }> }> {
  try {
    ...
    // Get HP data for ALL sites (for PIC Manager)
    const userHPData = allTasks
      .filter(task => {
        // For PIC Manager, show all tasks with HP > 0
        return task.hp && task.hp > 0;
      })
      .map(task => ({
        siteName: task.siteName || "-",
...
const Dashboard = () => {
  ...
  const filteredSiteData = useMemo(() => {
    ...
    // For PIC Manager, show ALL sites (no filtering by user)
    // No filtering needed - show all sites
